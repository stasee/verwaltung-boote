...
final class Plugin {
	...
	private function render_bootswart_table( $section ) {
		...
		if ( 'reservations' === $section ) {
			...
			?> <section><h2><?php esc_html_e( 'Alle Reservierungen', 'verwaltung-boote' ); ?></h2><div class="verwaltung-boote-tabellen-scroll"><table><thead><tr><th><?php esc_html_e( 'Boot', 'verwaltung-boote' ); ?></th><th><?php esc_html_e( 'Mitglied', 'verwaltung-boote' ); ?></th><th><?php esc_html_e( 'Start', 'verwaltung-boote' ); ?></th><th><?php esc_html_e( 'Ende', 'verwaltung-boote' ); ?></th><th><?php esc_html_e( 'Status', 'verwaltung-boote' ); ?></th><th><?php esc_html_e( 'Aktion', 'verwaltung-boote' ); ?></th></tr></thead><tbody><?php foreach ( $reservations as $reservation ) : $data = $this->get_reservation_data( $reservation->ID ); ?><tr><td><?php echo esc_html( $data['boat'] ); ?></td><td><?php echo esc_html( $data['user'] ); ?></td><td><?php $this->render_browser_datetime( $data['start_utc'] ); ?></td><td><?php $this->render_browser_datetime( $data['end_utc'] ); ?></td><td><?php echo esc_html( $data['status'] ); ?></td><td><?php if ( ! $data['log_id'] && ! $data['cancelled_utc'] && strtotime( $data['end_utc'] . ' UTC' ) >= time() ) : ?><form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>"><input type="hidden" name="action" value="verwaltung_boote_admin_reservierung_loeschen"><input type="hidden" name="reservierung_id" value="<?php echo esc_attr( $reservation->ID ); ?>"><input type="hidden" name="frontend" value="1"><?php wp_nonce_field( 'verwaltung_boote_admin_reservierung_loeschen_' . $reservation->ID ); ?><button type="submit"><?php esc_html_e( 'Stornieren', 'verwaltung-boote' ); ?></button></form><?php else : ?>–<?php endif; ?></td></tr><?php endforeach; if ( empty( $reservations ) ) : ?><tr><td colspan="6"><?php esc_html_e( 'Keine Reservierungen vorhanden.', 'verwaltung-boote' ); ?></td></tr><?php endif; ?></tbody></table></div></section><?php
			return;
		}

		if ( 'usages' === $section ) {
			$logs = get_posts( array( 'post_type' => 'vb_logbuch', 'post_status' => 'publish', 'posts_per_page' => -1, 'orderby' => 'date', 'order' => 'DESC' ) );
			?> <section><h2><?php esc_html_e( 'Alle Nutzungen', 'verwaltung-boote' ); ?></h2><div class="verwaltung-boote-tabellen-scroll"><table><thead><tr><th><?php esc_html_e( 'Boot', 'verwaltung-boote' ); ?></th><th><?php esc_html_e( 'Mitglied', 'verwaltung-boote' ); ?></th><th><?php esc_html_e( 'Ausleihe', 'verwaltung-boote' ); ?></th><th><?php esc_html_e( 'Rückgabe', 'verwaltung-boote' ); ?></th><th><?php esc_html_e( 'Reservierung', 'verwaltung-boote' ); ?></th><th><?php esc_html_e( 'Aktion', 'verwaltung-boote' ); ?></th></tr></thead><tbody><?php foreach ( $logs as $log ) : $data = $this->get_logbook_data( $log->ID ); ?><tr><td><?php echo esc_html( $data['boat'] ); ?></td><td><?php echo esc_html( $data['user'] ); ?></td><td><?php $this->render_browser_datetime( $data['start_utc'] ); ?></td><td><?php $this->render_browser_datetime( $data['end_utc'], __( 'Nutzung läuft', 'verwaltung-boote' ) ); ?></td><td><?php echo esc_html( $data['reservation_label'] ); ?></td><td><?php if ( ! $data['end_utc'] ) : ?><form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>"><input type="hidden" name="action" value="verwaltung_boote_admin_nutzung_beenden"><input type="hidden" name="log_id" value="<?php echo esc_attr( $log->ID ); ?>"><input type="hidden" name="frontend" value="1"><?php wp_nonce_field( 'verwaltung_boote_admin_nutzung_beenden_' . $log->ID ); ?><button type="submit"><?php esc_html_e( 'Nutzung beenden', 'verwaltung-boote' ); ?></button></form><?php else : ?>–<?php endif; ?></td></tr><?php endforeach; if ( empty( $logs ) ) : ?><tr><td colspan="6"><?php esc_html_e( 'Keine Nutzungen vorhanden.', 'verwaltung-boote' ); ?></td></tr><?php endif; ?></tbody></table></div></section><?php
			return;
		}
